...
- **Lean Core Desktop:** Zero bloat. Installs only `mangowc`, `noctalia`, `quickshell`, `kitty`, file management (`thunar`, `yazi`), terminal productivity utilities (`eza`, `htop`, `btop`, `zoxide`, `cava`), and essential Wayland display & recording utilities (`wlr-randr`, `wl-mirror`, `grim`, `slurp`, `gpu-screen-recorder`, `xdg-desktop-portal-wlr`). No NVIDIA proprietary builds, no OBS, and no heavy unnecessary apps.
- **Quickshell Ecosystem Ready:** Pre-configures the Quickshell repository/dependencies (`qt6-qtdeclarative`, `qt6-qtwayland`, `qt6-qt5compat`, `qt6-qtsvg`, `qt6-qtmultimedia`, `qt6-qtimageformats`, `socat`, `jq`) so you can run custom Quickshell bars, overviews, and widgets out of the box.
- **Virtual Machine Aware:** Automatically detects hypervisors (KVM, QEMU, Proxmox, VMware, VirtualBox) across all distros, installs `qemu-guest-agent`, enables the guest service, configures software cursor fallbacks (`WLR_NO_HARDWARE_CURSORS=1`), and masks missing hardware services to prevent timeouts.
